...
xnode_inf_x = [0 1/3 2/3 1 4/3 5/3 2 7/3 8/3 3];
xnode_inf_y = [0 1/3 2/3 1 4/3 5/3 2];
...
xnode_sup_x = [0 1/3 2/3 1];
xnode_sup_y = [2+1/3 2+2/3 3 3+1/3 3+2/3 4 4+1/3 4+2/3 5 5+1/3 5+2/3 6];
...
#model.maxit=100000;
#model.tol=0.0001;
#model.G=zeros(model.nnodes,1);
#for j = 1:length(xnode_inf_x)
  #for i = 1:length(xnode_inf_y)*3
  #    model.G((j-1)+i)= (0*xnode(j,1))+100;
  #endfor
#endfor
...
model.cp=1;
model.dt=0.05;
model.maxit=2/model.dt; #-> MAX IT es dt/2seg
model.tol=1e-5;


[PHI_FUENTE,Q_FUENTE]=fdm2d(xnode,icone,DIR,NEU,ROB,model); #Uso este PHI Y Q
fdm2d_graph_mesh(PHI_FUENTE,Q_FUENTE,xnode,icone,0,0);
disp('Fuente se apaga');

model.maxit=100000;
model.G=zeros(model.nnodes,1); #Fuente se apaga a los 2 seg
model.PHI_N=PHI_FUENTE; #PHI INICIAL ES EL DEL METODO ANTERIOR
